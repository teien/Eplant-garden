export default function card1() {
    return (
        <div>
            <div className=" mb-3" style={{
                maxwidth: '400px', marginTop: '80px'
            }}>
                <div className="row g-0">
                    <div className="col-md-4">
                        <img src="https://khothietke.net/wp-content/uploads/2021/04/PNGKhovector.net-02186.png" className="img-fluid rounded-start" alt="..." />
                    </div>
                    <div className="col-md-8">
                        <div className="card-body">
                            <h1 className="card-title">We Love Helping To Your Decorate Home Plants</h1>
                            <p className="card-text">Garden care is the process of maintaining and nurturning a garden to ensure its health, beauty,and productivity.</p>
                            <p className="card-text"><small className="text-muted">Last updated 3 mins ago</small></p>
                            <a className="btn btn-primary" href="#" role="button">Shop Now</a>
                        </div>
                    </div>
                </div>
            </div ><div className=" mb-3" style={{
                maxwidth: '540px', marginTop: '80px'
            }}>
                <div className="row g-0">
                    <div className="col-md-4">
                        <img src="https://khothietke.net/wp-content/uploads/2021/04/PNGKhovector.net-02186.png" className="img-fluid rounded-start" alt="..." />
                    </div>
                    <div className="col-md-8">
                        <div className="card-body">
                            <h2 className="card-title">We Love Helping To Your Decorate Home Plants</h2>
                            <p className="card-text">Garden care is the process of maintaining and nurturning a garden to ensure its health, beauty,and productivity.</p>
                            <p className="card-text"><small className="text-muted">Last updated 3 mins ago</small></p>
                            <a className="btn btn-primary" href="#" role="button">Shop Now</a>
                        </div>
                    </div>
                </div>
            </div >
        </div>


    );
}